...
export async function nsyncPlans() {
		...
		for (const product of products) {
			...
			for (const variant of variants) {
				...
				if (variant.attributes.status !== 'published') {
					console.log('Variante non publiée, on passe à la suivante')
					continue
				}

				// the first price is the newest one
				// we only need to sync the first price
